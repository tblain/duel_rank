...
export function exportCsv(state) {
  downloadText('classement-duelrank-v4.csv', 'text/csv;charset=utf-8', csvText(state));
}

export function exportTiers(state) {
  downloadText('tiers-duelrank-v4.csv', 'text/csv;charset=utf-8', tierCsvText(state));
}

export function exportMarkdown(state) {
  downloadText('classement-duelrank-v4.md', 'text/markdown;charset=utf-8', markdownText(state));
}

export function exportHtml(state) {
  downloadText('classement-duelrank-v4.html', 'text/html;charset=utf-8', htmlText(state));
}

export function exportSession(state) {
  downloadText('session-duelrank-v4.json', 'application/json;charset=utf-8', sessionJson(state));
}
